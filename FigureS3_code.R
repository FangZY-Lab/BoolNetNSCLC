##Robustness: activation levels
plist = list()
for(x in c(40,60,80,100)){
	d.EGFRmut_EGFRi50CTL25 = read.csv(unz(sprintf("result__Robust_on_APAF1procasp9__EGFRmutNSCLC__APAFproc%d.zip",x),'values.csv'))
	d.KRASmut_KRASi50CTL50 = read.csv(unz(sprintf("result__Robust_on_APAF1procasp9__KRASmutNSCLC__APAFproc%d.zip",x),'values.csv'))
...
cairo_pdf('fig_robust_on_fixed_vs_nonfixed.pdf',width=10,height=10)
print(p)
dev.off()

	#plot APAF-1 and procasp9 as well
plist = list()
for(x in c(40,60,80,100)){
	d.EGFRmut_EGFRi50CTL25 = read.csv(unz(sprintf("result__Robust_on_APAF1procasp9__EGFRmutNSCLC__APAFproc%d.zip",x),'values.csv'))
	d.KRASmut_KRASi50CTL50 = read.csv(unz(sprintf("result__Robust_on_APAF1procasp9__KRASmutNSCLC__APAFproc%d.zip",x),'values.csv'))
	...
	dtmp = lapply(dtmp,function(x){ x$iter=1:nrow(x); x })
	dtmp = lapply(dtmp,function(x){
		with(x, reshape2::melt(data.frame(iter=iter, APAF_1=APAF.1, pro_casp9=pro.casp9),id.vars='iter',variable.name='Protein',value.name='value'))
	})
	df = do.call(rbind,lapply(names(dtmp),function(x){ data.frame(cell_drug=x,dtmp[[x]]) }))
	df$cell = gsub('_.*','',df$cell_drug)
	df$drug = gsub('.*_','',df$cell_drug)
	...
	plist[[as.character(x)]] = pA
}

p = ggarrange(plist[[1]],plist[[2]],plist[[3]],plist[[4]],ncol=2,nrow=2,labels=LETTERS[1:4])+theme(plot.margin=unit(c(.1,1,.1,1),"cm"))

cairo_pdf('fig_robust_on_fixed_vs_nonfixed_APAF1procasp9.pdf',width=10,height=10)
print(p)
dev.off()
